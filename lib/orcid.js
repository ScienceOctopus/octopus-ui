...
function getPersonByOrcid(orcid, accessToken, callback) {
  const reqUrl = `https://pub.orcid.org/v2.1/${orcid}`;
  const reqOpts = {
    headers,
  };
...
function getPersonEducations(orcid, accessToken, callback) {
...
function getPersonEmployments(orcid, accessToken, callback) {
...
function getPersonWorks(orcid, accessToken, callback) {
  const reqUrl = `https://pub.orcid.org/v3.0/${orcid}/works`;
  const reqOpts = {
    headers,
  };
