...
function publicationTypesOptions(linksTo, type, linkedPublicationId, options) {
  const content = options.fn(this);
  let unacceptedValuesRegex;

  if (linkedPublicationId) {
    const acceptedPubTypes = ['PROBLEM'];

    if (type !== 'REVIEW') {
      acceptedPubTypes.push('REVIEW');
      const publicationTypes = options.data.root.publicationTypes;

      publicationTypes.forEach((publicationType, idx) => {
        if (publicationType.key === type) {
          const nextPubType = publicationTypes[idx + 1].key;
          acceptedPubTypes.push(nextPubType);
        }
      });
    }

    unacceptedValuesRegex = new RegExp(`value="((?!${acceptedPubTypes.join('|')}).)*"`, 'gi');
  } else {
    if (!linksTo || linksTo === '*') {
      return content;
    }
    unacceptedValuesRegex = new RegExp(`value="((?!${linksTo.join('|')}).)*"`, 'gi');
  }

  return content.replace(unacceptedValuesRegex, 'hidden disabled');
}

function findItemByKey(list = [], key, keyAlias) {
  return list.find((item) => item[keyAlias] === key);
}
