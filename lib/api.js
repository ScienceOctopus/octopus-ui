...
function findPublications(filters, callback) {
  ...
  filters.status = (typeof filters.status === 'string' ? filters.status.toUpperCase() : 'LIVE');
...
function findORCiDUser(orcid, callback) {
  const url = 'https://pub.orcid.org/v3.0/' + orcid + '/person';
  debug('octopus:ui:trace')(`Making call to ${url}`);

  return request.get(url, { headers: ORCiDHeaders }, (err, response, body) => {
    if (err) {
      console.error(err);
      return callback(err);
    }
...
function searchORCiDUsers(query, callback) {
