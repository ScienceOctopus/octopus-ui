...
function getReversion(publicationID, revision, callback) {
  const url = config.apiUrl + '/v1/reversions/getById/' + publicationID + '/' + revision;
  debug('octopus:ui:trace')(`Making call to ${url}`);
...
function createReversion(publicationData, callback) {
  const url = config.apiUrl + '/v1/reversions/create';
  debug('octopus:ui:trace')(`Making call to ${url}`);
